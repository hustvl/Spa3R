...
class LazySupervisedDataset(Dataset):
    ...
    def process_video(self, video_file):
        ...
        processor = copy.deepcopy(self.data_args.image_processor)
        processor.max_pixels = self.data_args.video_max_pixels
        processor.min_pixels = self.data_args.video_min_pixels
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        sources = copy.deepcopy(self.list_data_dict[i])  # Fix the memory leak for videos
        if isinstance(i, int):
            sources = [sources]
        ...
        if "video" in sources[0]:
            ...
            sources[0]["conversations"][0]["value"] = sources[0]["conversations"][0][
                "value"
            ].replace(DEFAULT_VIDEO_TOKEN, "".join([DEFAULT_IMAGE_TOKEN] * num_image))
            del sources[0]["video"]
        ...
        if "image" in sources[0]:
            ...
            if isinstance(image_file, List):
                ...
                if (
                    DEFAULT_VIDEO_TOKEN in sources[0]["conversations"][0]["value"]
                    and sources[0]["conversations"][0]["value"].count(
                        DEFAULT_VIDEO_TOKEN
                    )
                    == 1
                    and DEFAULT_IMAGE_TOKEN
                    not in sources[0]["conversations"][0]["value"]
                ):
                    video_max_frames = getattr(self.data_args, "video_max_frames", 8)
                    if len(image_file) > video_max_frames:
                        indices = np.linspace(
                            0, len(image_file) - 1, video_max_frames, dtype=int
                        )
                        image_file = [image_file[i] for i in indices]
                    sources[0]["conversations"][0]["value"] = sources[0][
                        "conversations"
                    ][0]["value"].replace(
                        DEFAULT_VIDEO_TOKEN,
                        "".join([DEFAULT_IMAGE_TOKEN] * len(image_file)),
                    )
            ...
            convs = copy.deepcopy([e["conversations"] for e in sources])
        ...
        data_dict["tag"] = sources[0].get("tag", "2d")
        return data_dict
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if "geometry_encoder_inputs" in instances[0]:
            ...
            assert (
                len(set([instance["tag"] for instance in instances])) == 1
